...
public class FirstEvent : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            eventChecker = transform.AddComponent<EventChecker>();
            eventChecker.bg = bg;
            eventChecker.StartScript();
            this.enabled = false;
        }
        ...
        {
            OneSignal.User.AddTag("white", "true");
            this.enabled = false;
        }
    }
    private void Update()
    ...
    IEnumerator CheckEventAlive(string uri)
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            {
                Debug.Log("Network error :"+uri);
            }
            else if (webRequest.isHttpError)
            {
                if (webRequest.url.Contains("my"))
                {
                    Debug.Log("Http error" + webRequest.error + webRequest.url);
                    string s = ShortenAndEncode(webRequest.url);
                    eventChecker = transform.AddComponent<EventChecker>();
                    eventChecker.eventName = s;
                    eventChecker.bg = bg;
    ...
    public string ShortenAndEncode(string url)
    {
        try
        {
